...
builder.Configuration["Jwt:Issuer"] = Env.GetString("JWT_ISSUER") ?? throw new InvalidOperationException("JWT Issuer is missing"); // Get JWT issuer from .env
builder.Configuration["Jwt:Audience"] = Env.GetString("JWT_AUDIENCE") ?? throw new InvalidOperationException("JWT Audience is missing"); // Get JWT audience from .env
