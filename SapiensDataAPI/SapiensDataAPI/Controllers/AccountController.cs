...
namespace SapiensDataAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        [HttpPut("public-like-admin/change-role")]
        public async Task<IActionResult> ChangeUserRole([FromBody] ChangeUserRoleRequestDto model)
        {
            // Validate input for username and role
            if (model == null || string.IsNullOrWhiteSpace(model.Username) || string.IsNullOrWhiteSpace(model.NewRole))
            ...
            if (!result.Succeeded)
                return BadRequest("Failed to change user role."); // Return error if role change fails

            // Return success after role change
            return Ok($"User role changed to {model.NewRole} successfully.");
        }
    }
}