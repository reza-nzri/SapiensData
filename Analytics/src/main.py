...
from utils import CLI, ChatGPTClient, JsonUpdater, FolderChecker, JsonFormatter, JsonReceiptRemover, \
                   MetadataExtractor, OcrExtractor, PDFTextExtractor, ReceiptSender, TextCleaner
...
def main(source_image):
    ...
    text_auto_corrector = TextAutoCorrector(username)
    text_auto_corrector.load_and_process_text()  # Auto-correct the text
    """
    ...
    # Step 12: Get the response from GPT based on OCR text
    response_text = gpt_client.get_response()
    
    # Step 13: Check if the response was successfully generated
    if response_text:
        print(Fore.YELLOW + "\nResponse has been saved as _temp_receipt.json." + Style.RESET_ALL)
    else:
        print(Fore.RED + "No response received." + Style.RESET_ALL)
    
    # Step 14: Create an instance of JsonFormatter for the given username and call the method to load, format, and save the JSON file
    ...
    # remove Reciept without contents    
